...
thread_local Uint	localThreadIndex{ 0u };
thread_local chrone::multithreading::scheduler::fiber::Fiber*	localPreviousFiber{};
thread_local chrone::multithreading::scheduler::fiber::Fiber*	localCurrentFiber{};
thread_local Uint	localcount{ 0u };
thread_local Uint	index{ 0u };
thread_local chrone::multithreading::scheduler::fiber::Fiber*	previousFiberBuffer[20]{};

static std::thread::id idArray[8]
{
...
FiberTaskScheduler::FiberTaskScheduler():
	_fibersData{ FiberTaskSchedulerProxy{this}, 
	...
	Uint orioginalThreadIndex)
{
	...
	assert(orioginalThreadIndex == localThreadIndex);
...
FiberTaskScheduler::_SwitchToFiber(Fiber* newFiber)
{
	...
	previousFiberBuffer[(index++) % 20] = fiberThreadData.previousThreadFiber;
	++localcount;
...
FiberTaskScheduler::_ExecuteAndTryFindFiber()
{
	...
	//Executing the current fiber task if there is one
